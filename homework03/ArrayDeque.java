...
public class ArrayDeque<T> {
     ...
     * Increments the size
     */
    private void resize(boolean addAtBack, T data) {
        T[] newArr = (T[]) new Object[backingArray.length * 2];
        int count = 0;
        while (count <= backingArray.length) {
            if (addAtBack) {
                newArr[count] = (count == backingArray.length) ? data : backingArray[mod(front + count, backingArray.length)];
            } else {
                newArr[count] = (count == 0) ? data : backingArray[mod(front + count - 1, backingArray.length)];
            }
            count++;
        }
        backingArray = newArr;
